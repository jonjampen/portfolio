...
<div class="max-w-4xl mx-auto py-8 animate-fade-in flex flex-col gap-8">
  ...
  <!-- Project Header Card -->
  <div class="rounded-2xl p-6 md:p-8 flex flex-col gap-6 relative overflow-hidden bg-neutral-900/20 backdrop-blur-xl hover:shadow-[0_0_30px_var(--color-primary-glow)] hover:-translate-y-0.5 transition-all duration-300">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-6 mt-2">
      <!-- Tags Badges -->
      <div class="flex flex-wrap gap-2">
        {#each metadata.tags as tag}
          <span class="text-xs bg-neutral-950 px-2.5 py-1 rounded text-neutral-450">{tag}</span>
        {/each}
      </div>
        ...
        <div class="flex items-center gap-3">
          {#if metadata.github}
            <a 
              ...
              class="flex items-center gap-2 bg-neutral-950 hover:bg-neutral-900 text-xs text-neutral-300 font-semibold px-4 py-2 rounded-xl transition-all select-none"
            >
    ...
    <!-- Impact box -->
    {#if metadata.impact}
      <div class="bg-primary/5 rounded-xl p-4 text-xs text-primary font-medium mt-2 leading-relaxed">
